...
Use this concise instruction for image synthesis. Add only source-specific scene facts and, if retrying, the smallest correction needed for the observed failure. Sections 1–10 remain mandatory for QC; do not append them mechanically to this prompt.

> Create only the lower artwork panel. Reconstruct the uploaded interior from exactly the same camera viewpoint as a faithful hand-printed archival interior illustration. Preserve the architecture, openings, hero furniture, major object count, silhouettes, positions, overlaps and depth order; do not redesign or add decor. Draw on warm ivory uncoated paper with irregular deep brown-black contours, directional engraved hatching, sparse cross-hatching and restrained dry-ink texture. Use exactly one muted spot color derived from the photograph. Keep generous visible paper around the drawing; it must not be full bleed. Add at most one tiny factual metadata cluster. No headline, masthead, photorealism, CGI, watercolor, clean vector art, generic retro advertising or fabricated facts.
